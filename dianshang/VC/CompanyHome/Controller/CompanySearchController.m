...
@interface CompanySearchController ()<YQCityLocationDelegate>
...
@property (nonatomic, strong) NSString       *keyword;// 期望职位名称
...
{
    //NSString *recommend = @"2";
    NSString *recommend = @"1";
    
    [self.hud show:YES];
    [[RequestManager sharedRequestManager] getCPersonnelList_uid:[UserEntity getUid] restatus:self.restatusStr recommend:recommend pname:self.keyword pay:self.payStr experience:self.exprienceStr edu:self.eduStr city:self.cityStr pagesize:KPageSize page:page success:^(id resultDic) {
        [self.hud hide:YES];
...
//    [[RequestManager sharedRequestManager] companySearch_uid:[UserEntity getUid] keyword:@"" work:self.exprienceStr edu:self.eduStr salary:self.payStr restatus:self.restatusStr page:page pagesize:KPageSize success:^(id resultDic) {
//        [self.hud hide:YES];
//        [self endRereshingCustom];
//        self.resultView.hidden = NO;
//        if ([resultDic[CODE] isEqualToString:SUCCESS]) {
...
{
    NSArray *array = [YQSaveManage objectForKey:LCHistoryRecord];
    if (array == nil) {
        array = [NSArray array];
    }
    return array;
...
- (void)saveHistoryRecord:(NSString *)str
{
    ...
    [YQSaveManage setObject:arr forKey:LCHistoryRecord];
}
