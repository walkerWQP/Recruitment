...
@interface CResumeDeliverController ()<UITableViewDelegate,UITableViewDataSource,UIScrollViewDelegate,XLAlertViewDelegate>
{
...
@implementation CResumeDeliverController
...
- (void)setNav
{
    ...
    if (titleArr.count > 1) {
        self.navigationItem.title = @"人才直投";
    }else{
...
- (void)buttonClick:(UIButton *)sender
{
    ...
    [UIView animateWithDuration:0.3 animations:^{
        self.headLineView.yq_centerX = button.yq_centerX;
    }];
    // 设置滑动的偏移量
...
{
    //0-人才简历;1-已面试;2-已入职;3-已转正;4-已离职
    ...
    /// dealwith(1表示人才简历；3、待面试; 5、已面试；6、已入职；7、已转正；8、已离职)
    [[RequestManager sharedRequestManager] getResumeDeliverList_uid:[UserEntity getUid] type:nil pclassid:self.pName dealwith:dealwithStr page:page pagesize:KPageSize success:^(id resultDic) {
