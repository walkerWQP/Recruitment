...
- (void)setupSubViews{
    ...
    CGFloat height = kActionItemHeight * (_otherBtnTitlesArr.count + 1) + 5 + APP_BottomH;
    KNActionSheetView *cancelView = [[KNActionSheetView alloc] init];
    [cancelView setDelegate:self];
    [cancelView setTag:-1];//lyq:改为取消永远为-1
    ...
    if (APP_BottomH > 0) {
        UIView *view = [[UIView alloc] initWithFrame:CGRectMake(0, CGRectGetMaxY(cancelView.frame), ScreenWidth, APP_BottomH)];
