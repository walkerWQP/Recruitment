...
- (void)_layoutSubviews
{
    ...
    NSString *userid = self.callSession.remoteName;
    // 根据扩展字段里的userid去数据库中查询
    UserInfo *userinfo = [UserInfo selectDataWithUserHXId:userid];
    if (userinfo != nil) {
        // 头像和昵称都存在数据库
        self.remoteNameLabel.text = userinfo.userName;
    }else{
...
- (void)timeTimerAction:(id)sender
{
    self.timeLength += 1;
    // 设置时长
    [DemoCallManager sharedManager].timeLength = self.timeLength;
...
- (IBAction)rejectAction:(id)sender
{
    ...
    [DemoCallManager sharedManager].isCallReject = YES;
...
- (IBAction)hangupAction:(id)sender
{
    ...
    [DemoCallManager sharedManager].isCallReject = NO;
