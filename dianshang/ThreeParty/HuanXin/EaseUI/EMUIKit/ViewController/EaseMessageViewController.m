...
@interface EaseMessageViewController ()<EaseMessageCellDelegate,YQCustomRecvWXMessageCellDelegate,YQCustomRecvResumeDelegate,YQCustomSendResumeDelegate>
{
...
- (void)setChatToolbar:(EaseChatToolbar *)chatToolbar
{
    ...
    tableFrame.origin.y = YQTopViewHeight;
    tableFrame.size.height = self.view.frame.size.height - iPhoneX_BOTTOM_HEIGHT - _chatToolbar.frame.size.height - YQTopViewHeight;
    ...
    if ([unreadMessages count])
    {
        ...
        for (EMMessage *message in unreadMessages)
        {
            [[EMClient sharedClient].chatManager sendMessageReadAck:message completion:^(EMMessage *aMessage, EMError *aError) {
                if (flag == 0) {
                    // lyq: 发送回执完成后主动设置tabbar.badgeValue
...
#warning 添加自定义的Cell
        ...
        NSString *lyqKey = [ext objectForKey:kKeyFlag];
        
        if ([lyqKey isEqualToString:kWXFlag] || [lyqKey isEqualToString:kTelFlag]) {
            NSString *flag = [model.message.ext objectForKey:kReplyFlag];
            // flag != -1 时要判断是同意还是拒绝 flag == -1时判断是发送方还是接收方发送方 height = 44 接收方 height = 90
            if ([flag isEqualToString:@"-1"]) {
                if (model.isSender) {
                    // 发送方显示
                    NSString *TimeCellIdentifier = [YQCustomSendWXMessageCell cellIdentifier];
                    YQCustomSendWXMessageCell *timeCell = (YQCustomSendWXMessageCell *)[tableView dequeueReusableCellWithIdentifier:TimeCellIdentifier];
                    
                    if (timeCell == nil) {
                        timeCell = [[YQCustomSendWXMessageCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:TimeCellIdentifier];
                        timeCell.selectionStyle = UITableViewCellSelectionStyleNone;
                    }
                    
                    timeCell.title = [model.message.ext objectForKey:kSendTextFlag];
                    return timeCell;
                }else{
                    // 显示带有同意/拒绝按钮的cell
                    NSString *CellIdentifier = [YQCustomRecvWXMessageCell cellIdentifier];
                    //send cell
                    YQCustomRecvWXMessageCell *sendCell = (YQCustomRecvWXMessageCell *)[tableView dequeueReusableCellWithIdentifier:CellIdentifier];
                    
                    // Configure the cell...
                    if (sendCell == nil) {
                        sendCell = [[YQCustomRecvWXMessageCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:CellIdentifier model:model];
                        sendCell.selectionStyle = UITableViewCellSelectionStyleNone;
                    }
                    sendCell.model = model;
                    sendCell.wxDelegate = self;
                    return sendCell;
                }
            }else{
                // 0->同意;1->拒绝
                if ([flag isEqualToString:@"0"]) {
                    // 同意后显示
                    NSString *TimeCellIdentifier = [YQCustomAgreeWXMessageCell cellIdentifier];
                    YQCustomAgreeWXMessageCell *timeCell = (YQCustomAgreeWXMessageCell *)[tableView dequeueReusableCellWithIdentifier:TimeCellIdentifier];
                    
                    if (timeCell == nil) {
                        timeCell = [[YQCustomAgreeWXMessageCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:TimeCellIdentifier];
                        timeCell.selectionStyle = UITableViewCellSelectionStyleNone;
                    }
                    if (model.isSender) {
                        timeCell.title = [model.message.ext objectForKey:kSendTextFlag];
                    }else{
                        timeCell.title = [model.message.ext objectForKey:kRecvTextFlag];
                    }
                    return timeCell;
        ...
        } else if ([lyqKey isEqualToString:kResumeFlag]){
            if (model.isSender) {
                // 发送方简历
                NSString *CellIdentifier = [YQCustomSendResumeCell cellIdentifier];
                //send cell
                YQCustomSendResumeCell *sendCell = (YQCustomSendResumeCell *)[tableView dequeueReusableCellWithIdentifier:CellIdentifier];
                
                // Configure the cell...
                if (sendCell == nil) {
                    sendCell = [[YQCustomSendResumeCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:CellIdentifier model:model];
                    sendCell.selectionStyle = UITableViewCellSelectionStyleNone;
                }
                sendCell.model = model;
                sendCell.sendResumeDelegate = self;
                return sendCell;
            } else {
                // 接收方简历
                NSString *CellIdentifier = [YQCustomRecvResumeCell cellIdentifier];
                //send cell
                YQCustomRecvResumeCell *sendCell = (YQCustomRecvResumeCell *)[tableView dequeueReusableCellWithIdentifier:CellIdentifier];
                
                // Configure the cell...
                if (sendCell == nil) {
                    sendCell = [[YQCustomRecvResumeCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:CellIdentifier model:model];
                    sendCell.selectionStyle = UITableViewCellSelectionStyleNone;
                }
                sendCell.model = model;
                sendCell.recvResumeDelegate = self;
                return sendCell;
            }
        }
        ...
        if (sendCell == nil) {
            ...
            sendCell.delegate = self;
////////////////////////////////////////////////////////////////////////////////////////////
            //////不知道更改的对不对   后期测试看看有什么bug没有
////////////////////////////////////////////////////////////////////////////////////////////
//            [sendCell.delegate avatarViewSelcted:model];
        }
...
- (CGFloat)tableView:(UITableView *)tableView heightForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if ([object isKindOfClass:[NSString class]]) {
        ...
        if ([lyqKey isEqualToString:kWXFlag] || [lyqKey isEqualToString:kTelFlag]) {
            ...
            if ([flag isEqualToString:@"-1"]) {
                if (model.isSender) {
                    return [YQCustomSendWXMessageCell cellHeight];
                }else{
                    return [YQCustomRecvWXMessageCell cellHeight];
                }
            }else{
                // 0->同意;1->拒绝
                if ([flag isEqualToString:@"0"]) {
                    return [YQCustomAgreeWXMessageCell cellHeight];
                }else if ([flag isEqualToString:@"1"]) {
                    return [YQCustomSendWXMessageCell cellHeight];
                }
            }
            ...
            if (model.isSender) {
                // 发送方简历高度
                return [YQCustomSendResumeCell cellHeight];
            }else{
                // 接收方简历高度
                return [YQCustomRecvResumeCell cellHeight];
            }
        }
...
#pragma mark - YQCustomRecvWXMessageCellDelegate
// 同意/拒绝按钮回调 0-同意;1-拒绝
- (void)wxButtonClick:(id<IMessageModel>)model buttonIndex:(NSInteger)buttonIndex
{
    if (_delegate && [_delegate respondsToSelector:@selector(messageViewController:didCellButtonSelectForModel:buttonIndex:)]) {
        [_delegate messageViewController:self didCellButtonSelectForModel:model buttonIndex:buttonIndex];
    }
}

#pragma mark - YQCustomSendResumeDelegate
- (void)sendResumeClick:(id<IMessageModel>)model
{
    if (_delegate && [_delegate respondsToSelector:@selector(messageViewController:didResumeSelectForModel:)]) {
        [_delegate messageViewController:self didResumeSelectForModel:model];
    }
}
#pragma mark - YQCustomRecvResumeDelegate
...
- (void)chatToolbarDidChangeFrameToHeight:(CGFloat)toHeight
{
    [UIView animateWithDuration:0.3 animations:^{
        ...
        rect.origin.y = YQTopViewHeight;
        rect.size.height = self.view.frame.size.height - iPhoneX_BOTTOM_HEIGHT - toHeight - YQTopViewHeight;
...
- (void)yq_sendImageMessage:(UIImage *)image withExt:(NSDictionary*)ext
{
    id progress = nil;
    if (_dataSource && [_dataSource respondsToSelector:@selector(messageViewController:progressDelegateForMessageBodyType:)]) {
        progress = [_dataSource messageViewController:self progressDelegateForMessageBodyType:EMMessageBodyTypeImage];
